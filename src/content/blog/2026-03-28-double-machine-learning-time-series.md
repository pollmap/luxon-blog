...
금융시장의 거시경제 충격(macro shock)을 정량화하려는 트레이더와 리서처들은 정교한 통계 모형을 필요로 한다. 특히 **정책 발표, 자금 흐름, 스테이블코인 공급 변화** 같은 외생적 사건이 자산 수익률, 펀딩 비용, 또는 basis에 미치는 인과적 효과를 추정할 때, 단순한 머신러닝 예측 모형으로는 부족하다. 

전통적인 Double Machine Learning(이중 머신러닝, DML)은 무작위로 샘플을 섞는 cross-fitting을 기반으로 설계되었다. 그러나 거시경제 시계열은 **시간에 따른 순차적 의존성(temporal dependence)**이 강하다. 지난달 금리가 이번달 금리에 영향을 미치고, 어제의 변동성이 오늘의 변동성을 예측한다. 무작위 샘플링은 이 순차 구조를 무너뜨린다. 

이 논문은 이 근본적인 문제를 직면하고, **시간 구조를 존중하는 역방향 교차검증(Reverse Cross-Fitting, RCF)**이라는 실행 가능한 해법을 제시한다. 금융 리서처에게 이는 거시 시계열을 다룰 때 인과추론의 신뢰성을 크게 높일 수 있는 방법론적 전환점이다.
...
이 절차는 i.i.d. 데이터에서는 정당화된다. 그러나 시계열에서는 **심각한 문제**가 발생한다:
...
이는 단순한 통계적 비효율이 아니라, **인과 추정 자체의 타당성**을 위협한다.
...
이 논문의 주요 기여는 **시간을 존중하는 폴드 설계**이다. 역방향 교차검증의 핵심 원리는:

1. **시간 순서 보존:** 폴드 A는 초기 시점부터 중간 시점까지, 폴드 B는 그 이후부터 말기 시점까지
2. **방향 반전:** 역방향 폴드(B에서 A로)도 함께 실행
3. **잔차 추정 안정성 확인:** fold-specific residualization이 일관되는지 검증

이를 통해:
- **표본 효율성 회복:** 근린 제거(neighbor-drop)나 과도한 절단(truncation) 없이도 유효한 추정
- **의존성 처리:** 시간 구조 내에서의 자연스러운 dependence를 허용
- **작은 표본 적응성:** 거시경제 월간 데이터(수십~수백 관측치)의 현실적 제약 수용
...
많은 실무자들이 머신러닝 모형을 선택할 때 **예측 오차(RMSE, MAE 등)**를 주된 기준으로 삼는다. 특히 cross-validation을 통해 "가장 낮은 예측 오차"를 보이는 모형을 nuisance 학습기로 채택한다.

그러나 이 논문은 **명확한 경고**를 제시한다:

> 최고의 예측 성능을 가진 모형이 최고의 인과 추정을 보장하지 않는다.

### 왜 이런 괴리가 발생하는가?

인과추론에서 nuisance 함수 $g_0$, $m_0$의 역할은 **confounding을 제거**하는 것이다. 반면 예측은 단순히 **조건부 평균을 맞추는** 것이다.

구체적 사례:
- **고차 다항식 또는 복잡한 비선형:** 개별 변수 간 미세한 패턴을 포착해 예측 RMSE를 낮춤
- **하지만:** 이 복잡성이 treatment 변수와의 상관 구조를 **과도하게 흡수**하여 실제 인과 계수를 누락
- **결과:** 낮은 예측 오차 ≠ 편향 없는 $\hat{\theta}$
...
**Goldilocks Zone**: 하이퍼파라미터 튜닝 시, 다음을 최적화하라:
- 개별 폴드별 residualization의 안정성 (fold-specific RMSE의 분산)
- 순방향과 역방향 폴드 간 추정값의 일관성
- 순수 예측 RMSE가 아닌, **orthogonalization 이후 잔차의 안정성**
...
DML의 orthogonalization 과정은 confounding을 제거하지만, **sequential dependence 자체는 남아있다**:
...
- 표준 표준오차: **편향됨** (too small)
- 신뢰구간: **명목 신뢰도보다 좁음** (과신뢰)

### HAC (Heteroskedasticity and Autocorrelation Consistent) 추론

해결책은 **Newey-West 스타일의 HAC 공분산 추정**:
...
HAC 추정 자체도 완벽하지 않다:
- **작은 표본:** bandwidth 선택의 민감성
- **구조적 단절:** 2008년 금융위기나 2020년 COVID 같은 regime shift는 모형의 가정 위반
- **비정상성:** 시계열이 unit root를 가지면 표준 inference 무효

따라서 DML+HAC를 사용할 때도 **사전에 안정성(stationarity) 검정**과 **부표본 분석(subsample stability check)**이 필수다.
...
Luxon의 거시 트레이딩 시스템은 **ETF 대량 유입/유출이 기초자산 수익률에 미치는 영향**을 정량화하려 한다. 현재는:
...
이 재설계는 거시 거래 신호의 **신뢰도를 25~40% 향상**시킬 것으로 예상된다 (표본 크기와 dependence 구조에 따라).
...
- **부분선형성:** $y_t = \theta_0 d_t + g_0(X_t) + \varepsilon_t$ 형태 성립
  - 검증: 변수 변환, 다항식 고정 후 잔차도 확인
- **약외생성(weak exogeneity):** $d_t$와 $\varepsilon_{t+1}$이 시점 $t$ 정보에서 직교
  - 현실: 정책 발표는 예상되지 않은(surprise) 부분만 $d_t$로 포함
- **stationarity:** 모든 변수가 I(0) 또는 공적분 관계
  - 비정상 시계열에는 추가 전처리 필요
...
1. **시계열 인과추론은 구조를 무시하면 안 됨:** 무작위 shuffling은 절대 금지
2. **예측 성능 ≠ 인과성 신뢰도:** 모형 선택 기준을 fold 안정성으로 이동
3. **작은 거시 샘플도 가능:** RCF로 양방향 정보 활용하면 표본 효율성 회복
4. **HAC inference 필수:** 신뢰구간의 명목 신뢰도 보장
